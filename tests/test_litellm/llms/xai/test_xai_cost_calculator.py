...
class TestXAIImageCostCalculator:
    ...
    @staticmethod
    def _image_cost(model, num_images):
    ...
    def test_image_cost_uses_per_image_rate(self):
        assert math.isclose(self._image_cost("xai/grok-imagine-image", 2), 2 * 0.02, rel_tol=1e-10)

    def test_image_quality_cost(self):
        assert math.isclose(self._image_cost("xai/grok-imagine-image-quality", 1), 0.05, rel_tol=1e-10)
...
class TestXAIGrokImagineCanonicalMapCost:
    ...
    def _video_response(duration_seconds):
        ...
        response.usage.provider_reported_cost_usd = None
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            ("xai/grok-imagine-image-quality", 1, 0.05),
        ],
    )
    def test_image_cost_from_canonical_map(self, model, num_images, expected):
        cost = TestXAIImageCostCalculator._image_cost(model, num_images)
        assert math.isclose(cost, expected, rel_tol=1e-10)
