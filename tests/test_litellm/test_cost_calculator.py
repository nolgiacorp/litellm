...
def test_completion_cost_bills_interactions_video_output_at_video_rate():
    ...
    # This fork registers gemini/gemini-omni-flash-preview as a video_generation entry (per-second
    # pricing behind /v1/videos), so the chat-shaped Omni entry with a video token rate is the bare
    # vertex_ai-language-models key; resolve it through vertex_ai to keep the interactions path covered.
    ...
    cost = completion_cost(completion_response=response, custom_llm_provider="vertex_ai")
